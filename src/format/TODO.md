...
These are documented divergences from gfortran's list-directed output. Output is functionally correct and readable; matching gfortran byte-for-byte requires more careful work. See `f2cl-lib:list-directed-descriptor` and `f2cl-lib:build-list-directed-format`.

- **3-digit exponents drop the `E`** — for `|x| >= 1d100` or `|x| < 1d-99`, the new printer emits ` 1.0000+100` rather than gfortran's `1.0000E+100`. This is standard Fortran behavior when the descriptor doesn't specify Ee; gfortran is the non-standard one. Fix: detect huge/tiny magnitudes and pick `1PGw.dE3` instead of plain `1PGw.d`. Cost: a runtime magnitude check per value.
- **COMPLEX components are padded** — output is ` (  1.5000000, -2.5000000)` rather than gfortran's `            (1.50000000,-2.50000000)`. The fixed-width `1PG14.7` per component produces internal padding; gfortran outputs each component at its natural width inside the parens with leading whitespace outside. Fix: bypass the format mini-language for complex and format `realpart` and `imagpart` with the scalar descriptor, then wrap manually. Or: build a wider format with leading X and tighter component widths.
...
- **Validate target test corpus** — list-directed is currently exercised only by `fmttest.f` and `ldtest.f`. The real test is retranslating quadpack with the flag on and diffing test output; that hasn't been done.
