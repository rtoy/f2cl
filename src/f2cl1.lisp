...
(defun f2cl-version ()
  (mapcar #'(lambda (s)
              ;; Need to strip off the leading "$Id:" so CVS/RCS won't
              ;; create a new Id.  Strip off the trailing "$" too.
              (string-right-trim "$"
...
(defparameter *intrinsic-function-names*
...
(defvar *non-intrinsic-function-names*
  nil
...
(defun print-header (outport declaim package options)
  ...
  (let ((version-info (f2cl-version)))
    (pprint-logical-block (outport version-info :per-line-prefix ";;; ")
      (format outport "Autogenerated by f2cl.  DO NOT EDIT~2%")
      (format outport "Report bugs to https://gitlab.common-lisp.net/f2cl/f2cl/-/issues~2%")
      (let ((versions (remove-if (lambda (v) (zerop (length v))) version-info)))
        (when versions
          (format outport "Compiled by f2cl version:~%")
          (dolist (v versions)
            (format outport "  ~A~%" v))))
      (format outport "~%Using Lisp ~A ~A~2%"
...
(defun fixup-f2cl-lib (tree external-names)
  ...
  (let ((external-names (append (remove-if #'(lambda (s)
...
;; Like check-reserved-lisp-names, but if the name would match an intrinsic, add it to the list of non-intrinsic names
(defun check-subprogram-arg-names (x)
  (when (member x *intrinsic-function-names*)
    (push x *non-intrinsic-function-names*))
