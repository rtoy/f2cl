...
C
      ...
      REAL A(NDIM),B(NDIM),EPSABS,EPSREL
      REAL RESULT(NUMFUN),ABSERR(NUMFUN)
      REAL VALUES(NUMFUN,MAXSUB),ERRORS(NUMFUN,MAXSUB)
      REAL CENTRS(NDIM,MAXSUB)
      REAL HWIDTS(NDIM,MAXSUB)
      REAL GREATE(MAXSUB),DIR(MAXSUB)
      REAL OLDRES(NUMFUN,MDIV)
      REAL WORK(LENW),RULPTS(WTLENG)
      REAL G(NDIM,WTLENG,2*MDIV),W(5,WTLENG)
      REAL CENTER(NDIM),HWIDTH(NDIM),X(NDIM,2*MDIV)
      REAL SCALES(3,WTLENG),NORMS(3,WTLENG)
C
C   Local variables.
C
...
C
      ...
      REAL OLDCEN,EST1,EST2,ERRCOF(6)
C
